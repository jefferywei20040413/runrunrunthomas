...
class MegaPiranhaPlant(pygame.sprite.Sprite):
    ...
    def attack2(self, all_sprites_group, platforms):
        print("Boss performs Attack 2: Summoning Ground Spikes!")
        # 在地面随机位置生成一些尖刺 (需要一个 Spike 类)
        # 遍历屏幕底部区域，寻找可以生成尖刺的位置
        spawn_area_left = self.rect.left - 100 # Boss左右一定范围内
        spawn_area_right = self.rect.right + 100
        ground_y = SCREEN_HEIGHT - GROUND_HEIGHT # 假设地面Y坐标

        spike_positions = []
        for _ in range(self.spike_count):
            x = random.randint(max(0, int(spawn_area_left)), min(SCREEN_WIDTH, int(spawn_area_right)))
            spike_positions.append((x, ground_y))

        for pos_x, pos_y in spike_positions:
            spike = Spike(pos_x, pos_y) # 你需要创建 Spike 类
